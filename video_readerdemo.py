...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def get_seqdemo(self, paths, idx=-1):
        n_frames = len(paths)
        if n_frames == self.args.seq_len:
            idxs = [int(f) for f in range(n_frames)]
        else:
        ...
        if (self.transform is not None):
            ...
            imgs = torch.stack(imgs)
        support_set = []
        support_set.append(imgs) 
        support_set = list(support_set)
        support_set = torch.cat(support_set).cuda()

        return support_set,idxs
    def get_seqpng(self, paths,idxs):
        ...
        imgs = [v for v in transform(imgs)]
