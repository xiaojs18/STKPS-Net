...
class Split():
    ...
    def add_vid(self, paths, gt_a,ttype):
        self.videos.append(paths)
        self.gt_a_list.append(gt_a)
        self.types.append(ttype)

    def get_rand_vid(self, label, mode,idx=-1):
        match_idxs = []
        for i in range(len(self.gt_a_list)):
            if label == self.gt_a_list[i] and mode==self.types[i]:
                match_idxs.append(i)
    ...
    def get_num_videos_for_class(self, label,mode):
        return len([gt for ii,gt in enumerate(self.gt_a_list) if gt == label and self.types[ii]==mode])
...
class VideoDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.supportnames=[x.strip().split('/')[1] for x in open("trainlist03.txt")]
        self.querynames=[x.strip().split('/')[1] for x in open("testlist03.txt")]
        #print(self.supportnames)
        ...
        if self.zip:
            ...
            for class_folder in class_folders:
                ...
                for video_folder in video_folders:
                    ...
                    if video_folder in self.supportnames:
                     c.add_vid(paths, class_id,'support')
                     print(video_folder)
                    elif video_folder in self.querynames:
                     c.add_vid(paths, class_id,'query')
                     print(video_folder)
                    else:
                     c.add_vid(paths, class_id,'others')
        #print(c.types)
    ...
    def get_seq(self, label, mode,idx=-1):
        c = self.get_train_or_test_db()
        paths, vid_id = c.get_rand_vid(label,mode,idx) 
        n_frames = len(paths)
    ...
    def __getitem__(self, index):
        ...
        for bl, bc in enumerate(batch_classes):
           if self.train: 
            ...
            n_total_support = c.get_num_videos_for_class(bc,'support')
            idxs = random.sample([i for i in range(n_total_support)], self.args.shot)
            n_total_query = c.get_num_videos_for_class(bc,'query')
            idxp = random.sample([i for i in range(n_total_query)],  n_queries)

            for idx in idxs[0:self.args.shot]:
                vid, vid_id = self.get_seq(bc, 'support',idx)
                support_set.append(vid)
                support_labels.append(bl)
            for idx in idxp[:n_queries]:
                vid, vid_id = self.get_seq(bc, 'query',idx)
                target_set.append(vid)
                target_labels.append(bl)
                real_target_labels.append(bc)    
