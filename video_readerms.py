...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def setup_transforms(self):
        ...
        video_transform_list_small = []
        video_test_list = []
        video_test_list_small=[]
        
        video_transform_list_small.append(Resize(256))#
        video_test_list_small.append(Resize(256))#
        ...
        video_transform_list_small.append(RandomHorizontalFlip())
        video_transform_list_small.append(RandomCrop(224))#

        video_test_list.append(CenterCrop(self.img_size))
        video_test_list_small.append(CenterCrop(224))#

        self.transform = {}
        self.transform["train"] = Compose(video_transform_list)
        self.transform["train_small"] = Compose(video_transform_list_small)
        self.transform["test"] = Compose(video_test_list)
        self.transform["test_small"] = Compose(video_test_list_small)
    ...
    def _select_fold(self):
        ...
        #print(self.train_test_lists["train"])
    ...
    def get_seq(self, label, idx=-1):
        ...
        n_frames = len(paths)
        if n_frames == self.args.seq_len//4:
            idxs = [int(f) for f in range(n_frames)]
        else:
            if self.train:
                excess_frames = n_frames - self.seq_len//4
            ...
            if end - start < self.seq_len//4:
                end = n_frames - 1
                start = 0
            else:
                pass
    
            idx_f = np.linspace(start, end, num=self.seq_len//4)
        ...
        seq_len_b=(self.args.seq_len*3)//4
        if n_frames == seq_len_b:
            idxb = [int(f) for f in range(n_frames)]
        else:
            if self.train:
                excess_frames = n_frames - seq_len_b
                excess_pad = int(min(5, excess_frames / 2))
                if excess_pad < 1:
                    start = 0
                    end = n_frames - 1
            ...
            if end - start < seq_len_b:
                end = n_frames - 1
                start = 0
            else:
                pass
    
            idx_fb = np.linspace(start, end, num=seq_len_b)
            idxb = [int(f) for f in idx_fb]
            
            if seq_len_b == 1:
                idxb = [random.randint(start, end-1)]        
        imgb = [self.read_single_image(paths[i]) for i in idxb]
        imgs = [self.read_single_image(paths[i]) for i in idxs]
        if (self.transform is not None):
            if self.train:
                transform = self.transform["train"]
                transform_small = self.transform["train_small"]
            else:
                transform = self.transform["test"]
                transform_small = self.transform["test_small"]
            imgs = [self.tensor_transform(v) for v in transform_small(imgs)]
            imgs = torch.stack(imgs)
            imgb = [self.tensor_transform(v) for v in transform(imgb)]
            imgb = torch.stack(imgb)
        return imgb, imgs, vid_id
    ...
    def __getitem__(self, index):
        ...
        for bl, bc in enumerate(batch_classes):
            ...
            for idx in idxs[0:self.args.shot]:
                vid, vid_s, vid_id = self.get_seq(bc, idx)
                support_set.append(vid)
                support_set_s.append(vid_s)
                support_labels.append(bl)
            for idx in idxs[self.args.shot:]:
                vid, vid_s, vid_id = self.get_seq(bc, idx)
                target_set.append(vid)
                target_set_s.append(vid_s)
                target_labels.append(bl)
                real_target_labels.append(bc)
        
        s = list(zip(support_set, support_set_s,support_labels))
        random.shuffle(s)
        support_set, support_set_s, support_labels = zip(*s)
        
        t = list(zip(target_set, target_set_s, target_labels, real_target_labels))
        random.shuffle(t)
        target_set, target_set_s, target_labels, real_target_labels = zip(*t)
        
        support_set = torch.cat(support_set)
        target_set = torch.cat(target_set)
        support_set_s = torch.cat(support_set_s)
        target_set_s = torch.cat(target_set_s)
        #print((support_set))
        #print(support_set_s.size())
        #print(target_set_s.size())
        ...
        return {"support_set":support_set, "support_set_s":support_set_s,"support_labels":support_labels, "target_set":target_set,"target_set_s":target_set_s, "target_labels":target_labels, "real_target_labels":real_target_labels, "batch_class_list": batch_classes}
