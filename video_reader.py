...
class VideoDataset(torch.utils.data.Dataset):
        ...
        if self.zip:
            ...
            for class_folder in class_folders:
                ...
                for video_folder in video_folders:
                    # print(f"\n正在处理: {video_folder}")
                    # print(f"  调用 get_train_or_test_db('{video_folder}')")
                    #
                    # # 添加这些调试信息
                    # print(f"  train_test_lists['train']的前3个: {self.train_test_lists['train'][:3]}")
                    # print(f"  video_folder in train列表: {video_folder in self.train_test_lists['train']}")
                    # print(f"  video_folder in test列表: {video_folder in self.train_test_lists['test']}")
                    ...
                    # print("c",c)
                    if c is None:
                        # print(f"Skipping {video_folder} - not in train/test lists")
                        continue
                    # if c == None:
                    #     continue
    ...
    def get_train_or_test_db(self, split=None):
        if split is None:
            ...
            # print("train_split",self.train_split)
            # print("split",self.train_test_lists["train"])
    ...
    #                     selected_files.append(video_name)
    #
    #         lists[name] = selected_files
    #
    #     self.train_test_lists = lists
