...
Experimental Setup Hardware: 4 GPUs (--num_gpus 4) Setting: 5-way 10-shot Memory Optimization: Due to the high memory cost of 10-shot settings, query_per_class is set to 2 during training and testing.
...
`CUDA_VISIBLE_DEVICES`	GPU Allocation. Specifies which GPUs to use. Example: 0,1 uses the first two GPUs. Ensure this matches --num_gpus.

`--dataset`	Dataset Selection.

`-c / --checkpoint_path`	  Workspace Directory. Path to save logs and checkpoints, or to load them from.

`-r`	Resume / Evaluate. If flagged, the model attempts to load weights from checkpoint.pt inside the directory specified by -c.

`--shot`	Support Set Size. Number of support samples per class (e.g., 1, 5, 10).

`--num_gpus`	GPU Count. Number of GPUs used for Distributed Data Parallel .

`--query_per_class`	Training Batch Size. Number of query samples per class during training.

`--query_per_class_test`	Testing Batch Size. Number of query samples per class during validation/testing.