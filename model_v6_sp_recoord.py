...
from adafocus_v6_sp_recoord import AdaFocus
...
class TemporalCrossTransformer(nn.Module):
    def __init__(self, args, mode,temporal_set_size=3):
        ...
        if (mode=='all'):
         ...
         self.pe = PositionalEncoding((self.args.trans_linear_in_dim//2), self.args.trans_dropout, max_len=max_len)
         self.k_linear = nn.Linear((self.args.trans_linear_in_dim//2) * temporal_set_size, (self.args.trans_linear_out_dim))#.cuda()
         self.v_linear = nn.Linear((self.args.trans_linear_in_dim//2)* temporal_set_size, (self.args.trans_linear_out_dim))#.cuda()
         self.norm_k = nn.LayerNorm((self.args.trans_linear_out_dim))
         self.norm_v = nn.LayerNorm((self.args.trans_linear_out_dim))
...
class CNN_TRX(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.transformers= nn.ModuleList([TemporalCrossTransformer(args, 'all',s) for s in args.temp_set]) 
    ...
    def forward(self, context_images, context_labels, target_images, mode):
        ...
        all_logits_global = [t(context_features_global, context_labels, target_features_global)['logits'] for t in self.transformers_global]
        ...
        all_logits_local = [t(context_features_local, context_labels, target_features_local)['logits'] for t in self.transformers_local]
        ...
        return_dict = {'logits': split_first_dim_linear(sample_logits, [NUM_SAMPLES, target_features.shape[0]]),'logits_global': split_first_dim_linear(sample_logits_global, [NUM_SAMPLES, target_features_global.shape[0]]),'logits_local': split_first_dim_linear(sample_logits_local, [NUM_SAMPLES, target_features_local.shape[0]])}
        return return_dict
